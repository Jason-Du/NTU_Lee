...
def trainMBGD(arrayTrainX, arrayTrainY, intBatchSize, intEpochSize, floatLearnRate):
    ...
    for epoch in range(intEpochSize):
        ...
        if epoch > 0:
            # train cost
            arrayTrainCost = arrayTotalLoss / (len(arrayTrainX))#平均iter的Loss
...
if __name__ == "__main__":
    ...
    dfTrainX = pd.read_csv(os.path.join(os.path.dirname(__file__), "X_train.csv"))
    dfTrainY = pd.read_csv(os.path.join(os.path.dirname(__file__), "Y_train.csv"))
    dfTestX = pd.read_csv(os.path.join(os.path.dirname(__file__), "X_test.csv"))
    ...
    print(arrayNormalizeTrainX)
    print(arrayNormalizeTrainX.shape)
    print("---------------------")
    ...
    arrayW, arrayB = trainMBGD(arrayTrainX, arrayTrainY, intBatchSize=32, intEpochSize=50, floatLearnRate=0.001)
